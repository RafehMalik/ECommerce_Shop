...
func (s *ProductService) CreateCateory(req *dto.CreateCategoryRequest) (*dto.CategoryResponse, error) {
	cateory := models.Category{
		Name:        req.Name,
		Description: req.Description,
	}
	if err := s.db.Create(&cateory).Error; err != nil {
		return nil, err
	}
	return &dto.CategoryResponse{
		...
		IsActive:    cateory.IsActive,
	}, nil
}

func (s *ProductService) GetCategory() ([]dto.CategoryResponse, error) {
	var categories []models.Category
	err := s.db.Where("is_ative=?", true).Find(&categories).Error
	if err != nil {
		return nil, err
	}
...
func (s *ProductService) CreateProduct(req *dto.CreateProductRequest) (*dto.ProductResponse, error) {
	...
	return s.GetProduct(uint(prduct.ID))
}
